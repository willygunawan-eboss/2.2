import { MutateEmployeeDTO } from './dto/MutateEmployeeDTO';
...
export class MutateEmployeeUseCase {
  constructor(
  ...
  async execute(dto: MutateEmployeeDTO): Promise<any> {
    ...
    return await this.unitOfWork.execute(async ({ employeeRepo, employmentRepo, assignmentRepo, orgRepo, posRepo }) => {
      ...
      WorkforceDomainService.validateMutation(employee, activeAssignment, org, pos, dto.effectiveDate);
      ...
      try {
        const policyResult = await this.policyService.evaluatePolicyByCode('MUTATION_APPROVAL', policyContext);
      ...
      const { updatedOldAssignment, newAssignment } = WorkforceDomainService.processMutation(
        activeAssignment!,
      ...
      await this.auditService.recordAudit(activeEmployment.id, "CREATE_ASSIGNMENT", dto.actor, { assignmentId: newAssignment.id, reason: dto.reason, type: "MUTATION" });
      await this.timelineService.recordTimeline(activeEmployment.id, "MUTATED", dto.actor, { reason: dto.reason, effectiveDate: dto.effectiveDate, newPositionId: dto.newPositionId });
      ...
      await this.eventPublisher.publish("EmployeeMutated", payload, traceId);
