...
import { TransferEmployeeUseCase } from './TransferEmployeeUseCase';
...
describe('TransferEmployeeUseCase', () => {
  ...
  let useCase: TransferEmployeeUseCase;
  ...
  it('should transfer employee successfully', async () => {
  ...
  it('should throw error if effective date is before current assignment', async () => {
    const errorUseCase = new TransferEmployeeUseCase(
      {
        execute: async (fn: any) => {
          return fn({
    ...
    await expect(errorUseCase.execute({
      ...
      reason: 'Promotion',
      actor: 'HRAdmin'
    })).rejects.toThrow("Transfer effective date must be after current assignment effective date");
  });
});
