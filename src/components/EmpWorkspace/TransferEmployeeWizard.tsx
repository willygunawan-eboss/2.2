...
interface TransferEmployeeWizardProps {
  onClose: () => void;
  onSuccess: () => void;
}

export function TransferEmployeeWizard({ onClose, onSuccess }: TransferEmployeeWizardProps) {
  ...
  useEffect(() => {
    // In a real app we'd fetch employees matching searchQuery
    // For now, mock a few or fetch from API
    // Let's just create a list of employees for the dropdown/search
    setEmployees([
    ...
    // Fetch orgs and positions (mocked for now as we did in HireEmployeeWizard if we don't have endpoints ready)
    setOrganizations([
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      const response = await fetch('/api/workforce/transfer', {
        method: 'POST',
      ...
      if (!response.ok) throw new Error(data.message || 'Failed to transfer employee');
      onSuccess();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      >
        <div className="p-6 bg-white border-b border-slate-200 flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-xl font-semibold text-slate-900">Transfer Employee</h2>
            <p className="text-sm text-slate-500 mt-1">Initiate a formal employee transfer process</p>
          </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-slate-700 mb-1">Transfer Reason <span className="text-red-500">*</span></label>
                  <textarea name="reason" value={formData.reason} onChange={handleChange} rows={3} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Explain the reason for this transfer..."></textarea>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="bg-white border border-slate-200 rounded-lg p-5 space-y-4">
                ...
                <div className="pt-4 border-t border-slate-100 flex items-start gap-3 bg-blue-50/50 p-3 rounded-lg">
                  ...
                  <p className="text-xs text-blue-800">
                    Executing this transfer will automatically terminate the current assignment and create a new one. Depending on business policy, this may trigger an approval workflow.
                  </p>
                </div>
              </div>
            </div>
