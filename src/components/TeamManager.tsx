...
export default function TeamManager() {
  ...
  useEffect(() => {
    ...
    if (did) fetch(`/api/org/sections?limit=100&departmentId=${did}`).then(res => res.json()).then(res => setSectionsData(res.data)).catch(console.error);
  ...
  const fetchEntities = async () => {
    ...
    setIsError(false);
    try {
      let url = `/api/org/teams?page=${page}&limit=10&search=${search}&showDeleted=${showDeleted}`;
      ...
      if (sectionIdFilter) url += `&sectionId=${sectionIdFilter}`;
...
const response = await res.json();
  ...
  useEffect(() => {
    fetchEntities();
  }, [page, search, showDeleted, companyIdFilter, branchIdFilter, divisionIdFilter, departmentIdFilter, sectionIdFilter]);
  
  useEffect(() => {
    if (isAuditModalOpen && selectedTeam) {
      fetch(`/api/org/teams/${selectedTeam.id}/audits`).then(res => res.json()).then(res => setAuditsData(res.data)).catch(console.error);
    }
  }, [isAuditModalOpen, selectedTeam]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedTeam) {
        await fetch(`/api/org/teams/${selectedTeam.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) });
      } else {
        await fetch(`/api/org/teams`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) });
      }
  ...
  const deleteMutationMutate = async (id: string) => {
    try {
      await fetch(`/api/org/teams/${id}`, { method: 'DELETE' });
      fetchEntities();
    } catch(err) {
  ...
  const restoreMutationMutate = async (id: string) => {
    try {
      await fetch(`/api/org/teams/${id}/restore`, { method: 'POST' });
      fetchEntities();
    } catch(err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 flex flex-col gap-4">
        <div className="flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="showDeletedTeams" className="text-sm text-gray-600">Show Deleted</label>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-10 text-red-500">Error loading teams</div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="px-6 py-4 text-sm font-medium text-gray-600">Section</th>
            ...
            <tbody>
              {data?.data?.map((team: any) => (
                <tr key={team.id} className={`border-b border-gray-50 hover:bg-gray-50 ${team.deletedAt ? 'bg-red-50 hover:bg-red-100' : ''}`}>
                  <td className="px-6 py-4">
                    <span className="font-mono text-sm text-gray-600">{team.code}</span>
                  </td>
                  <td className="px-6 py-4 font-medium text-gray-900">{team.name}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{team.sectionName}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {team.departmentName}
                    <div className="text-xs text-gray-400">{team.divisionName}</div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 text-xs rounded-full ${team.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                      {team.status || 'Active'}
                    </span>
                    {team.deletedAt && (
                  ...
                  <td className="px-6 py-4 flex justify-end gap-2">
                    <button
                      onClick={() => { setSelectedTeam(team); setIsAuditModalOpen(true); }}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setFormData({ ...formData, companyId: e.target.value, branchId: '', divisionId: '', departmentId: '', sectionId: '' });
                    }}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setFormData({ ...formData, branchId: e.target.value, divisionId: '', departmentId: '', sectionId: '' });
                    }}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setFormData({ ...formData, divisionId: e.target.value, departmentId: '', sectionId: '' });
                    }}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setFormData({ ...formData, departmentId: e.target.value, sectionId: '' });
                    }}
