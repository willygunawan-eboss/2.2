...
export function OrganizationHealth() {
  ...
  if (loading || !data) return <div className="text-slate-400 p-8">Loading Health Data...</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-medium text-white">Platform Health & Integrity</h2>
        <button 
          onClick={refreshCache}
          disabled={refreshing}
          className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded border border-slate-700 text-sm transition-colors"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 flex flex-col items-center justify-center">
          <div className="text-slate-400 font-medium mb-4">Integrity Score</div>
          ...
          <div className="space-y-3 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
            {data.errors.length > 0 ? (
              data.errors.map((err: string, i: number) => (
                <div key={i} className="text-sm text-slate-300 p-2 bg-slate-750 rounded border border-red-500/20">
                  {err}
                </div>
              ))
            ) : (
          ...
          <div className="space-y-3 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
            {data.warnings.length > 0 ? (
              data.warnings.map((warn: string, i: number) => (
                <div key={i} className="text-sm text-slate-300 p-2 bg-slate-750 rounded border border-yellow-500/20">
                  {warn}
                </div>
              ))
            ) : (
