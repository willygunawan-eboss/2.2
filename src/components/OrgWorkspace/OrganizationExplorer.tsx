...
function TreeNode({ node, level, expanded, onToggle, searchQuery }: any) {
  ...
  return (
    <div className="select-none">
      ...
      >
        <span className="w-5 h-5 flex items-center justify-center mr-1 text-slate-500 group-hover:text-slate-700">
          {hasChildren && (
        ...
        <span className={clsx("text-sm", matchSearch ? "text-blue-600 font-medium" : "text-slate-700")}>
          {node.name}
        </span>
...
export function OrganizationExplorer() {
  ...
  if (loading) return <div className="text-slate-500 p-8">Loading Tree...</div>;

  return (
    <div className="bg-white border border-slate-200 rounded-lg p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-slate-900 font-medium">Organization Explorer</h3>
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none w-64"
            />
          </div>
          <button onClick={expandAll} className="px-3 py-1.5 text-sm bg-slate-700 hover:bg-slate-600 text-slate-900 rounded">Expand All</button>
          <button onClick={collapseAll} className="px-3 py-1.5 text-sm bg-slate-700 hover:bg-slate-600 text-slate-900 rounded">Collapse All</button>
        </div>
      </div>
